...
class Tracker:
    ...
    def interpolate_ball_positions(self, ball_positions):
        ...
        df_ball_positions = df_ball_positions.bfill() #to fill some missing frames incase the above cant 
    ...
    def get_object_tracker(self, frames, read_from_stub=False, stub_path=None):
        ...
        tracks = {
            "players": [], #for each frame we have id and then bbox for that id 
            "referees": [], 
            "ball": []
        }
        ...
        for frame_num, detection in enumerate(detections):
            ...
            #covert detection to supervision detection format
            detection_supervision = sv.Detections.from_ultralytics(detection)

            #convert goaley to player object
    ...
    def draw_ellipse(self, frame, bbox, color, track_id=None):
        ...
        if track_id is not None:
            cv2.rectangle(frame, (int(x1_rect), int(y1_rect)), (int(x2_rect), int(y2_rect)), 
            color, cv2.FILLED)
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        #transparent rectagle and show stats
        overlay = frame.copy()
        cv2.rectangle(overlay, (1350, 850), (1900, 970), (255, 255, 255), cv2.FILLED)
        alpha = 0.4
        cv2.addWeighted(overlay, alpha, frame, 1-alpha, 0, frame)

        team_ball_control_tillFrame = team_ball_control[:frame_num+1]
        #Get the number of times each team had the ball
        team1_num_frames = team_ball_control_tillFrame[team_ball_control_tillFrame==1].shape[0]
        team2_num_frames = team_ball_control_tillFrame[team_ball_control_tillFrame==2].shape[0]

        team1 = team1_num_frames/(team1_num_frames+team2_num_frames)
        team2 = team2_num_frames/(team1_num_frames+team2_num_frames)

        cv2.putText(frame, f"Team 1 Ball Possession: {team2*100:.2f}%", (1355, 900), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), thickness=3)
        cv2.putText(frame, f"Team 2 Ball Possession: {team1*100:.2f}%", (1355, 950), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), thickness=3)
